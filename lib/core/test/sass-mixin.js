...
// Test mixin
describe('./src/base/mixin', function() {
    let file = path.join(__dirname, '../src/base/_mixin.scss');
    let sassaby = new Sassaby(file, {
       dependencies: [
           path.join(__dirname, '../src/base/_function.scss')
       ]
    });
    ...
    describe('#position (快捷设置元素postion的混入)', function() {
        ...
        it('应该在传参`absolute, 2em`调用时返回绝对定位且四个方向的距离值都是2em', function() {
            sassaby.includedMixin('position').calledWithArgs('absolute').declares('position', 'absolute');
            sassaby.includedMixin('position').calledWithArgs('absolute', '2em').declares('top', '2em');
            sassaby.includedMixin('position').calledWithArgs('absolute', '2em').equals(
                'position: absolute; ' +
                'top: 2em; ' +
                'right: 2em; ' +
                'bottom: 2em; ' +
    ...
    describe('#triangle (三角型生成器)', function() {
        it('应该在生成"向下"三角形时不含有下边框的规则声明, 且上边框颜色不透明', function() {
            sassaby.includedMixin('triangle').calledWithArgs('10px, #333, down').doesNotDeclare('border-bottom', '5px solid #333');
            sassaby.includedMixin('triangle').calledWithArgs('10px, #333, down').declares('border-top', '5px solid #333');
        });

        it('应该在生成"向上"三角形时不含有上边框的规则声明, 且下边框颜色不透明', function() {
            sassaby.includedMixin('triangle').calledWithArgs('10px, #333, up').doesNotDeclare('border-top', '5px solid transparent');
            sassaby.includedMixin('triangle').calledWithArgs('10px, #333, up').declares('border-bottom', '5px solid #333');
        });
    });
    ...
    describe('#ellipsis (单行截取溢出字符)', function() {
        it('应该在传入指定400px宽度时最大宽度 "400px"', function() {
            sassaby.includedMixin('ellipsis').calledWithArgs('400px').declares('max-width', '400px');
        });
    ...
    describe('#center-tl (居中元素, transform版本)', function() {
        it('直接调用时应该水平垂直居中', function() {
            sassaby.includedMixin('center-tl').called().declares('transform', 'translate(-50%,-50%)');
        ...
        it('$horizontal: false; 调用时应该垂直居中', function() {
            sassaby.includedMixin('center-tl').calledWithArgs('".inner", false').declares('transform', 'translateY(-50%)');
            sassaby.includedMixin('center-tl').calledWithArgs('".inner", false').declares('top', '50%');
        });
    });
    ...
    describe('#center-td (居中元素, table版本)', function() {
        ...
        it('$horizontal: false; 调用时应该垂直居中', function() {
            sassaby.includedMixin('center-td').calledWithArgs('".inner", false').doesNotDeclare('text-align', 'center');
        });
    });
});
